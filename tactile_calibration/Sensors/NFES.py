...
class FSRStreamSensor(Sensor):
    ...
    def __init__(self, port: str = "COM5", baud: int = 115200, buffer_size: int = 200):
        super().__init__()
    ...
    def capture_image(self, duration_s: float) -> Tuple[np.ndarray, np.ndarray]:
        """
        Return (t_seconds_forward, values) for the last duration_s seconds,
        where t runs from ~0 to duration_s based on timestamps.
        """
        now = time.time()
        cutoff = now - float(duration_s)
        with self._lock:
        ...
        t_rel = ts[mask] - now  # negative up to 0
        vals = ys[mask]
        # Convert to forward axis 0..duration
        t = t_rel - t_rel[0]
        t = t - t[0]
        return t, vals
    ...
    def save_window_image(self, duration_s: float = 10.0, out_path: str = "fsr_last10.png",
        ...
        """
        t, vals = self.capture_window_by_duration(duration_s)
        if t.size == 0:
...
def main():
    ...
    p = argparse.ArgumentParser(description="FSR A0 live plotting (AnalogD2-style refactor)")
